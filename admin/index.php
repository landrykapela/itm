...
if(isset($_POST['submit'])){
    ...
    $location = "Location: ".((isset($_SERVER['HTTPS']) && strtolower($_SERVER['HTTPS']) == 'on') ? "https://":"http://") .$_SERVER['HTTP_HOST']."/jobs/signup.html?fb=".$response;
    ...
    if($action){
        session_start();
        $_SESSION['user'] = $email;
        $location = "Location: ".((isset($_SERVER['HTTPS']) && strtolower($_SERVER['HTTPS']) == 'on') ? "https://":"http://") .$_SERVER['HTTP_HOST']."/admin/profile.php";
        header($location,true);
    }
    else {
